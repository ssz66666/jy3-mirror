...
function t:详细显示()
    ...
    if  G.misc().book == 1 then 
        ...
        for i = 1,6 do
            if o_role_人物[tostring(i+2)] > 150 and not o_role_人物[tostring(901)] then
    ...
    elseif  G.misc().book == 3 then
        ...
        str = {'无','中毒','麻痹','晕眩','内伤','受伤','迟缓','中毒受伤','致盲','混乱','斩杀','剧毒','绝杀'}
        if o_skill.类别 < 6 or o_skill.招式  then 
            self.武功.getChildByName('攻击效果').text = '攻击效果：[01]'..str[o_skill.附加效果+1]
        else
            self.武功.getChildByName('攻击效果').text = '攻击效果：[01]无'
        end
        str = {'加血(主动技能)','杀内','吸内(上限)','减伤','闪避','复活(主动技能)','强化','免疫中毒','回气','回血','回内','吸内','反伤','免疫封穴','免疫内伤','御风(主动技能)','纯阳','武穆','回血清异常(主动技能)'}
        ...
        if o_skill.类别 == 9 then
            local str_阵法效果 = {'每名存活队友降低敌人内功防御5%','每名存活队友增加慈悲效果10%','每名存活队友增加剑法伤害5%','每名存活队友增加内功回复效果1%','有存活队友情况下或者会左右互搏玉女素心剑法增加破防效果','每名存活队友增加10%控制几率','全队友存活时任何单体攻击变为攻击群体','中毒效果变为剧毒效果，根据存活队友人数增加中毒时序','每名存活队友增加主角5%的舔血吸血效果','每名存活队友增加逍遥御风的持续时间'}
            local str_名称 = {'真武七截阵','金刚伏魔阵','五岳剑阵','天罡北斗七星阵','玉女剑阵','五行八卦阵','打狗阵','逍遥毒仙阵','万灵血阵','逍遥御仙阵'}
            for i = 1,#str_名称 do
