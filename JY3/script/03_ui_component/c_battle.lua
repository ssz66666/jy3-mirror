...
function t:start()
    ...
    if o_battle.模式 > 3 and o_battle.模式 ~= 99 then
...
function t:刷新显示()
    ...
    local i_role = 0x10040000
    local o_battle = G.QueryName(i_battle)
    if o_battle.模式 < 4 or o_battle.模式 == 99 then 
...
function t:战场显示()
    ...
    for i = 1,4 do 
        if o_hotkey[tostring(10+i)] ~= nil and G.QueryName(o_hotkey[tostring(10+i)]).数量 > 0 then 
            ...
            if G.misc().战斗状态 == 0 and  self.obj.getChildByName('map').getChildByName(位置[1]).x <= 150  and G.call('get_point',87) == 0 and G.call('get_point',44) > 0  and (o_battle.模式 < 4 or o_battle.模式 == 99  )   then 
                if G.misc().用药 == 0  then 
    ...
    for i = 2,11 do
        ...
        if tar == self.透明按钮.getChildByName(位置[i]) and o_role_人物.生命 > 0 then 
            ...
            if o_role_人物 ~= nil then 
                ...
                for i = 1,6 do
                    if G.call('get_role',int_队员编号,i+2) > 150 and not o_role_人物[tostring(901)] then
