...
function t:队友属性()
    ...
    if o_role_人物 ~= nil then 
        ...
        local magic = {'破绽','慈悲','先攻','妙手','急速','冰心','暴击','激励','见切','万毒','强体','回春','强力','强行','复生','奇才','活力','阴毒','舔血','北冥','真武','朱雀','玄武','青龙','白虎','指心','拳劲','剑意','刀魂','奇门','暗日','寒气','绝杀','剑神','灭绝','武神'}
        for i = 1,4 do 
    ...
    for i = 7,10 do 
        if tar == self.装备图.getChildByName(类别[i]) and self.装备图.getChildByName(类别[i]).img ~= 0x56161002 then
            ...
            if o_equip.特效 > 0 and o_equip.类型 == math.floor(o_equip.特效/100)  then 
                ...
                local 特效说明_4 = {'附加被动【指心】','附加被动【拳劲】','附加被动【剑意】','附加被动【刀魂】','附加被动【奇门】','附加被动【暗日】','附加被动【激励】','附加被动【寒气】','附加被动【强行】','附加被动【暴击】','附加额外生命内力与被动【口才】','附加额外生命内力与被动【武神】'}
                local str_特效说明 = ''
            ...
            if o_equip.套装 > 0 then 
                ...
                elseif o_equip.套装 == 3 then
                    ...
                    str = str..'[br]'
                    if G.call('通用_取得套装',0,o_equip.套装) >= 2 then
                        str = str..'[br][07](二件套:)[br][06]'..'生命+2500 内力+2500  伤害+15%，治疗效果+15%，命中率增加'
                    else
                        str = str..'[br][09](二件套:)[br][09]'..'生命+2500 内力+2500  伤害+15%，治疗效果+15%，命中率增加'
                    end
                    str = str..'[br]'
                    if G.call('通用_取得套装',0,o_equip.套装) >= 3 then
                        str = str..'[br][07](三件套:)[br][06]'..'生命+5000 内力+5000 伤害+30%，治疗效果+30%，突破上限，攻击必定命中'
                    else
                        str = str..'[br][09](三件套:)[br][09]'..'生命+5000 内力+5000 伤害+30%，治疗效果+30%，突破上限，攻击必定命中'
                    end
