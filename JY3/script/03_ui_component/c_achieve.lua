...
function t:start()
    ...
    for x = 1, 3, 1 do
        for y = 1, 6, 1 do
            local ui_sub_1 = G.Clone(self.进度.getChildByName(tostring(24)).getChildByName('周目'))
            local ui_sub_2 = G.Clone(self.进度.getChildByName(tostring(25)).getChildByName('套装'))
            ui_sub_1.x = -190 + (x - 1) * 200 
            ui_sub_1.y = 120 + (y - 1) * -60
            ui_sub_2.x = -190 + (x - 1) * 200 
            ui_sub_2.y = 120 + (y - 1) * -60
            self.进度.getChildByName(tostring(24)).addChild(ui_sub_1)
            self.进度.getChildByName(tostring(25)).addChild(ui_sub_2)
            ui_sub_1.visible = false
            ui_sub_2.visible = false
            self.list_24[(y - 1) * 3 + x] = ui_sub_1
            self.list_25[(y - 1) * 3 + x] = ui_sub_2
        end
    end 
    for x = 1, 4, 1 do
        for y = 1, 6, 1 do
    ...
    for x = 1, 3, 1 do
        for y = 1, 4, 1 do
            local ui_sub = G.Clone(self.进度.getChildByName(tostring(26)).getChildByName('人物'))
            ui_sub.x = -30 + (x - 1) * 200 
            ui_sub.y = -10 + (y - 1) * -90
            self.进度.getChildByName(tostring(26)).addChild(ui_sub)
            ui_sub.visible = false
            self.list_26[(y - 1) * 3 + x] = ui_sub
        end
    end 
    for x = 1, 3, 1 do
        for y = 1, 4, 1 do
            local ui_sub = G.Clone(self.进度.getChildByName(tostring(27)).getChildByName('人物'))
            ui_sub.x = -30 + (x - 1) * 200 
            ui_sub.y = -10 + (y - 1) * -90
            self.进度.getChildByName(tostring(27)).addChild(ui_sub)
            ui_sub.visible = false
            self.list_27[(y - 1) * 3 + x] = ui_sub
        end
    end 
    G.misc().data = 0
...
function t:更新()
    ...
    if self.按钮.getChildByName('子菜单').visible  then
        self.进度.getChildByName('left').visible = false 
        self.进度.getChildByName('right').visible = false 
        self.进度.getChildByName('left1').visible = false 
        self.进度.getChildByName('right1').visible = false 
    else
        self.进度.getChildByName('left').visible = true
        self.进度.getChildByName('right').visible = true 
        self.进度.getChildByName('left1').visible = true 
        self.进度.getChildByName('right1').visible = true 
    end
    if self.按钮.getChildByName('主菜单').visible or self.按钮.getChildByName('子菜单').visible then
        self.进度.getChildByName('页数').visible = false 
    else
        self.进度.getChildByName('页数').visible = true
    end
end
...
function t:click(tar)
    ...
    if tar == self.说明.getChildByName('详细') then 
        ...
        local no = {3,6,12,13,14,15,16,18,19,22,23,24,25,26,27,28,29}
        self.进度.getChildByName('总页数').text = 1
        self.进度.getChildByName('页数').text = 1
        local point = 0
        local maxpoint = 0
    ...
    if  G.misc().data == 28 then
        ...
        for i = 1,#o_achieve.进度列表 do
            self.进度.getChildByName(tostring(G.misc().data)).getChildByName(tostring(i)).visible = true
            if o_achieve.进度列表[i].完美 then 
                self.进度.getChildByName(tostring(G.misc().data)).getChildByName(tostring(i)).getChildByName('闪光').visible = true
            else
                self.进度.getChildByName(tostring(G.misc().data)).getChildByName(tostring(i)).getChildByName('闪光').visible = false 
            end
            if o_achieve.进度列表[i].完成 == 0 then
                self.进度.getChildByName(tostring(G.misc().data)).getChildByName(tostring(i)).getChildByName('完成').img = 0x56160073 
                self.进度.getChildByName(tostring(G.misc().data)).getChildByName(tostring(i)).getChildByName('名称').style = 10
    ...
    if  G.misc().data == 15 or G.misc().data == 16 or G.misc().data == 22 or G.misc().data == 26 or G.misc().data == 27 then 
        local o_achieve = G.QueryName(0x10170000+G.misc().data) 
    ...
    if  G.misc().data == 24 then
        local o_achieve = G.QueryName(0x10170000+G.misc().data) 
        for i = 1,#o_achieve.进度列表 do 
            self.list_24[i].getChildByName('名称').text = o_achieve.进度列表[i].名称
            self.list_24[i].visible = true
            if o_achieve.进度列表[i].完成 == 0 then 
                self.list_24[i].getChildByName('完成').img = 0x56160073
                self.list_24[i].getChildByName('名称').style = 10
            else
                self.list_24[i].getChildByName('完成').img = 0x56160072
                self.list_24[i].getChildByName('名称').style = 5
            end  
        end 
    end
    if  G.misc().data == 25 then
        local o_achieve = G.QueryName(0x10170000+G.misc().data) 
        for i = 1,#o_achieve.进度列表 do 
            self.list_25[i].getChildByName('名称').text = o_achieve.进度列表[i].名称
            self.list_25[i].visible = true
            if o_achieve.进度列表[i].完成 == 0 then 
                self.list_25[i].getChildByName('完成').img = 0x56160073
                self.list_25[i].getChildByName('名称').style = 10
            else
                self.list_25[i].getChildByName('完成').img = 0x56160072
                self.list_25[i].getChildByName('名称').style = 5
            end  
        end 
    end
    ...
    if  G.misc().data == 26 then
        ...
        for i = 1,12 do 
            self.list_26[i].visible = false
        end  
        for i = 1,显示数量 do 
            self.list_26[i].getChildByName('名称').text = o_achieve.进度列表[(ys-1)*12+i].名称
            self.list_26[i].getChildByName('头像').img = 0x56080000+o_achieve.进度列表[(ys-1)*12+i].编号
            self.list_26[i].visible = true
            if o_achieve.进度列表[(ys-1)*12+i].完成 == 0 then 
                self.list_26[i].getChildByName('完成').img = 0x56160073
                self.list_26[i].getChildByName('名称').style = 10
                self.list_26[i].getChildByName('闪光').visible = false
            else
                self.list_26[i].getChildByName('完成').img = 0x56160072
                self.list_26[i].getChildByName('名称').style = 5
                self.list_26[i].getChildByName('闪光').visible = true
            end    
        end 
    end 
    if  G.misc().data == 27 then
        local o_achieve = G.QueryName(0x10170000+G.misc().data) 
        if ys < zys then
            显示数量 = 12
        else
        ...
        for i = 1,12 do 
            self.list_27[i].visible = false
        end  
        for i = 1,显示数量 do 
            self.list_27[i].getChildByName('名称').text = o_achieve.进度列表[(ys-1)*12+i].名称
            self.list_27[i].getChildByName('头像').img = 0x56080000+o_achieve.进度列表[(ys-1)*12+i].编号
            self.list_27[i].visible = true
            if o_achieve.进度列表[(ys-1)*12+i].完成 == 0 then 
                self.list_27[i].getChildByName('完成').img = 0x56160073
                self.list_27[i].getChildByName('名称').style = 10
                self.list_27[i].getChildByName('闪光').visible = false
            else
                self.list_27[i].getChildByName('完成').img = 0x56160072
                self.list_27[i].getChildByName('名称').style = 5
                self.list_27[i].getChildByName('闪光').visible = true
            end    
        end 
    end 
