...
t['战斗系统_自动战斗'] = function()
    while true do
        ...
        if r == 1 then
            ...
            ui = G.getUI('v_battle')
            local 位置 = {'team1','team2','team3','team4','team5','enemy1','enemy2','enemy3','enemy4','enemy5','enemy6'}
            if ui.getChildByName('map').getChildByName(位置[1]).x == 150 and G.misc().选择目标 == 0  then
                                ...
                                elseif 	o_skill.内功轻功效果 == 6 then   --全体复活
                                    ...
                                    for p = 2,5 do 
                                        if o_battle[位置[p] ] > 0 then
                                            if G.QueryName(o_role +o_battle[位置[p]] ).生命 <= 0 then 
                                                n = n + 1
                                                int_hp = math.max(1,math.floor(int_hp/2))
...
t['集气'] = function()
	...
	while true do 
        ...
        for i = 2,11 do 
            ...
            if o_battle[位置[i] ] > 0 then
                if  o_role_npc.生命 > 0 then 
                    ...
                    if G.call('get_role',int_role,90) > 0 then --npc剧毒
                        G.call('add_role',int_role,15,-math.max(1,math.floor(25/int_比例)))  
                    end  
        ...
        if G.call('get_point',81) > 0  and G.call('get_point',90) == 0 then  --中毒剧毒受伤
            G.call('add_point',44,-math.max(1,math.floor(15/int_比例)))
        end
        if G.call('get_point',90) > 0 then  
            G.call('add_point',44,-math.max(1,math.floor(25/int_比例)))
        end
...
t['magic_power1'] = function(int_id,int_no)
    ...
    if i_magic_阵法 then 
        ...
        elseif G.QueryName(i_magic_阵法).附加效果 == 10 then 
            int_毒仙阵效果 = int_队友
        elseif G.QueryName(i_magic_阵法).附加效果 == 11 then
            int_诛仙效果 = int_队友
    ...
    if math.random(a+20) > math.random(math.floor(b/3)+1+int_闪避) or G.call('通用_取得装备特效',0,208) or o_skill.附加效果 == 14 or o_skill.附加效果 == 13 or G.call('通用_取得套装',0,3) == 3 or  G.call('通用_取得人物特效',0,35)  then  --命中计算     
        if hurt > 0 then 
            ...
            if  o_skill.附加效果 == 14  then 
                ...
                hurt = hurt 
            elseif math.random(100) > 80 and (G.call('通用_取得人物特效',0,22) or G.call('通用_取得装备特效',0,206) )  then --朱雀被动效果
                if string_字符串_4 == '' then 
            ...
            --强伤（灭绝判断）
            if G.call('通用_取得人物特效',0,36) and  G.call('通用_取得套装',int_id,4) ~= 3  then  
                if math.random(100) < 50 - int_诛仙效果*10 then 
                    if string_字符串_4 == '' then 
            ...
            if  o_skill.附加效果 == 15  then 
                if math.random(seed) < 5 + math.floor(G.call('get_point',179+27)/10) then 
                    if not G.call('get_role',int_id,240) or G.call('get_role',int_id,240) == 0  then 
                        if string_字符串_4 == '' then 
                        ...
                        G.call('set_role',int_id,250,int_时序)
                    end 
                end
            end
        ...
        if hurt > 0 then 
           ...
           -- hurt = hurt * (100-G.call('通用_取得NPC内功效果',int_enemy,4)/2)/100 --内功特效减伤
            if  o_skill.附加效果 == 14  then 
                ...
                hurt = hurt 
            elseif math.random(100) > 80 and (G.call('通用_取得人物特效',int_id,22) or G.call('通用_取得装备特效',int_id,206))  then --朱雀被动效果
                if string_字符串_4 == '' then 
                    string_字符串_4 = string_字符串_4..'朱雀'
                else
                    string_字符串_4 = string_字符串_4..'.'..'朱雀'
            ...
            elseif G.call('通用_取得人物特效',int_id,21) then --真武效果
                int_减伤 = int_减伤 + c/4 + c/6*0.8+G.call('通用_取得装备减伤效果',int_enemy)
                --hurt = math.floor(hurt *(1- c/400)*(1- d/600*0.6)*(1 - G.call('通用_取得装备减伤效果',int_enemy)/200 )  ) 
            else
                int_减伤 = int_减伤 + c/4 + c/6+G.call('通用_取得装备减伤效果',int_enemy)
            ...
            --强伤（灭绝判断）
            if G.call('通用_取得人物特效',int_id,36) and G.call('通用_取得套装',int_enemy,4) ~= 3  then 
            ...
            if  o_skill.附加效果 == 15  then 
                if math.random(seed) < 5+ math.floor(G.call('get_role',int_id,6)/10) then 
                    if not G.call('get_role',int_enemy,240) or G.call('get_role',int_enemy,240) == 0  then 
                        if string_字符串_4 == '' then 
                            string_字符串_4 = string_字符串_4..'强伤'
                        else
                            string_字符串_4 = string_字符串_4..'.'..'强伤'
...
t['magic_power3'] = function(int_id,int_no)
    ...
    if math.random(a+20) > math.random(math.floor(b/3)+int_闪避 + 1)  or G.call('通用_取得装备特效',int_id,208) or o_skill.附加效果 == 14 or o_skill.附加效果 == 13  or G.call('通用_取得套装',int_id,3) == 3 or  G.call('通用_取得人物特效',int_id,35)  then  --命中计算
        if hurt > 0 then 
            ...
            --强伤（灭绝判断）
            if G.call('通用_取得人物特效',int_id,36) and G.call('通用_取得套装',0,4) ~= 3  then 
                if math.random(100) < 25 then 
                    if string_字符串_4 == '' then 
                        string_字符串_4 = string_字符串_4..'灭绝'
                    else
            ...
            if  o_skill.附加效果 == 15  then 
                if math.random(seed) < 5 + math.floor(G.call('get_role',int_id,6)/10 - G.call('get_point',21)/10) then 
                    if not G.call('get_ponit',240) or G.call('get_ponit',240) == 0  then 
                        if string_字符串_4 == '' then 
                            string_字符串_4 = string_字符串_4..'强伤'
                        else
                            string_字符串_4 = string_字符串_4..'.'..'强伤'
