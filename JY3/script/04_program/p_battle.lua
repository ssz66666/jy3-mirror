            ...
            if ui.getChildByName('map').getChildByName(位置[1]).x >= 330 then   --主角战斗逻辑
                ...
                if  o_skill == G.QueryName(0x10050000+207) then --只有普通攻击才为混乱状态
                    ...
                    if o_skill.范围 > 1 then 
                        ...
                        G.noti_call('战场_效果',1,int_动作编号,int_动画位置)   
                    end 
                end     		
    ...
    if math.random(a+15) > math.random(math.floor(b/6)+int_闪避+1) then  --命中计算
        ...
        if hurt > 0 then 
            ...
            elseif G.call('通用_取得人物特效',int_id,21) then --真武效果
                ...
                hurt = math.floor(hurt *(1- c/400)*(1- d/400)*(1 - G.call('通用_取得装备减伤效果',int_enemy)/200 ) )  --按敌人的拆招和内功免伤进行计算伤害
            end
    ...
    if math.random(a+15) > math.random(math.floor(b/6)+int_闪避 + 1) then  --命中计算
        if hurt > 0 then 
            ...
            elseif G.call('通用_取得人物特效',int_id,21)then
                hurt = math.floor(hurt *(1- c/500)*(1- d/800)*(1 - G.call('通用_取得装备减伤效果',0)/100 )   )
            else
                hurt = math.floor(hurt *(1- c/500)*(1- d/800*0.5)*(1 - G.call('通用_取得装备减伤效果',0)/200 )   )  --按主角的拆招和内功免伤计算
            end
