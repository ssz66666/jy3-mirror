...
t['地图系统_防修改还原'] = function()
	...
	if G.call('get_point',63) ~= math.abs(G.call('get_newpoint',63) + 10)  then
		...
		G.call('set_point',63, math.abs(G.call('get_newpoint',63) + 10)) 
		--int_检测 = int_检测 + 1 
	end
...
t['地图系统_防修改监控'] = function()
	...
	for i = 1,#G.QueryName(0x10170012).进度列表 do 
        if G.QueryName(0x10170012).进度列表[i].完成 == 1 then
            int_周目成就 = int_周目成就 + 1
        end
    end
	...
	for i = 1,#G.QueryName(0x1017001d).进度列表 do 
        if G.QueryName(0x1017001d).进度列表[i].当前进度 == 1 then
            int_周目成就 = int_周目成就 + 1
        end
    end
	...
	if #o_cardhouse.卡片 > 0 then 
		...
		for i = 1,#o_cardhouse.卡片 do
			...
			if o_cardhouse.卡片[i].数量 > 1 and o_card.品级 <= 2 then
				int_卡片超量 = 1
			end
		end
		if int_卡片数量 ~= math.abs(G.call('get_newpoint',83)+1000) or int_卡片超量 == 1  then 
			--print('物品数量',int_物品数量,G.call('get_newpoint',76))
			int_检测 = int_检测 + 1
		end
	end
...
t['挖矿时间条'] = function()
	...
	while true do 
		...
		--G.misc().小游戏时间 = ui2.getChildByName('时间').width
		if G.misc().小游戏时间 ~= math.abs(G.misc().小游戏检测时间 +10) and G.misc().修改锁定检测_1 == 0 then 
			G.misc().修改锁定检测_1 = 1
		end
	end 
end	
...
t['切磋'] = function()
	...
	if math.random(o_role_人物[tostring(9)]) < (math.random(30) + 30 )  then 
		...
		if o_battle_结果  == 1 then 
			if not G.call('通用_是否满属性',o_role_人物代码) then
				G.call('add_exp',o_role_人物代码,math.random(1000,3000)) 
			end
		else	
