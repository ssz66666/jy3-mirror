...
t['通用_选择自动攻击武功']=function()
    ...
    if G.QueryName(0x100c0001)[tostring(G.misc().自动技能序号)] then
        if G.QueryName(G.QueryName(0x100c0001)[tostring(G.misc().自动技能序号)]).类别 == 5 and G.call('get_point',198) == nil then
            else
    ...
    for i = 7,1,-1 do 
        if G.QueryName(0x100c0001)[tostring(i)] then
            if G.QueryName(G.QueryName(0x100c0001)[tostring(i)]).类别 == 5 and G.call('get_point',198) == nil then
            else
