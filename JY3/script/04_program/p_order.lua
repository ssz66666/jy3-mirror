...
t['通用_读档'] = function(int_档案编号)
	...
	if G.IsFileExist(path) then
        ...
        if int_档案编号 > 0 and int_档案编号 <= 4 then
            ...
            if  maxpoint == 6655 and point == 6655 then
                G.call('通用_强退游戏') 
            end
...
t['通用_随机抽取卡片']=function()
    ...
    for i = 1,5 do
        ...
        elseif int_随机数 > 250 and int_随机数 <= 1000 then 
            int_卡组 = 6
        elseif int_随机数 > 1000 and int_随机数 <= 3000 then 
            int_卡组 = 3
        elseif int_随机数 > 3000 and int_随机数 <= 6000 then 
            int_卡组 = 4
        elseif  int_随机数 <= 10000 then 
            int_卡组 = 5
        end
            ...
            if c.卡区.getChildByName('card_'..(int_比拼位置+1)).getChildByName('卡片').visible and c.卡区.getChildByName('card_'..(int_比拼位置+3)).getChildByName('卡片').visible then 
                if (c.cardmod[int_比拼卡].防御 +  c.cardmod[int_比拼卡].智慧) == c.cardmod[tonumber(_编号[int_比拼位置+1])].速度 + c.cardmod[tonumber(_编号[int_比拼位置+3])].力量 then
                    if int_归属 ~= c.cardmod[tonumber(_编号[int_比拼位置+1])].归属 and int_右 == 0 then 
                        int_右 = 1
                        int_翻牌 = int_翻牌 + 1
                        if G.call('get_cardgame_lv') >= 9 then
                            print('翻开',1)
                            int_翻牌 = int_翻牌 + G.call('get_cardgame_pick_next',tonumber(_编号[int_比拼位置+1]),int_比拼位置+1,int_归属) 
                        end
                    end
                    if int_归属 ~= c.cardmod[tonumber(_编号[int_比拼位置+3])].归属 and int_下 == 0 then 
                        ...
                        if G.call('get_cardgame_lv') >= 9 then
                            print('翻开',2)
                            int_翻牌 = int_翻牌 + G.call('get_cardgame_pick_next',tonumber(_编号[int_比拼位置+3]),int_比拼位置+3,int_归属) 
                        end
                    end
                end
...
t['get_cardgame_pick_next'] = function(int_比拼卡,int_比拼位置,int_归属)
    local int_翻牌 = 0
    local ui = G.getUI('v_cardgame')
    local c = ui.c_cardgame
    print('记录',int_比拼卡,int_比拼位置)
    local o_card = c.cardmod[int_比拼卡].o_card_卡片
        --卡片比拼计算
   -- local int_归属 = c.cardmod[int_比拼卡].归属
    local _编号 = {}
...
t['call_cardgame_pick_next'] = function(int_比拼卡,int_比拼位置)
    if G.call('get_cardgame_lv') < 10 then
        return 
    end
    local ui = G.getUI('v_cardgame')
    local c = ui.c_cardgame
            ...
            if c.卡区.getChildByName('card_'..(int_比拼位置+1)).getChildByName('卡片').visible and c.卡区.getChildByName('card_'..(int_比拼位置+3)).getChildByName('卡片').visible then
                if c.cardmod[int_比拼卡].防御 + c.cardmod[tonumber(_编号[int_比拼位置+1])].速度 == c.cardmod[int_比拼卡].智慧 + c.cardmod[tonumber(_编号[int_比拼位置+3])].力量 then
                    if int_归属 ~= c.cardmod[tonumber(_编号[int_比拼位置+1])].归属 then 
                        ...
                        G.call('call_cardgame_pick_next',tonumber(_编号[int_比拼位置+1]),int_比拼位置+1)
                    end
...
t['call_cardgame'] = function()
    ...
    while true do
        ...
        if c.进程 == 4 and c.已放置 < 9 then
            ...
            c.二区.getChildByName('card_'..int_卡位置).getChildByName('背景').visible = false
            c:zoomOut(int_放置卡)
...
t['小游戏_华容道'] = function()
    ...
    if  G.misc().拼图结果 == 1 and G.misc().计时器 < 870 then 
...
t['重生']=function() 
    ...
    local int_修改检测 = G.misc().修改锁定检测_3
    ...
    local o_equip_usb = {}
    local i_equip
    if #table_继承装备 > 0 and int_生存 ~= 1  and  G.misc().修改锁定检测_3 ~= 1 then 
...
t['call_wood']=function() 
    ...
    elseif int_选项 == 2 then
        ...
        G.call('set_role',223,14,9989999)
        G.call('set_role',223,1,9989999)
        G.call('set_role',223,2,9989999)
        ...
        G.call('set_role',223,14,40000)
        G.call('set_role',223,1,40000)
        G.call('set_role',223,2,40000)
        G.QueryName(0x10040000+223).存储记录 = -10-G.call('get_role',223,1)
...
t['add_point']=function(int_代码,int_数量) --增加主角部分属性
    ...
    if int_代码 == 3 then   --经验升级计算
        ...
        if G.call('get_point',3) >= int_升级经验  and int_升级经验 > 0 and G.call('get_point',4) < int_lvmax  then 
            ...
            local int_随机种子 = G.call('通用_取宝物随机')
...
t['通用_发放礼包']=function()
    if G.misc().新年礼包 == nil or (G.misc().新年礼包 == 1 and G.call('get_point',4) < 50) then
