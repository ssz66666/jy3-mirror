	...
	while int_选项 == 0 do
        ...
        if int_选项 == 1 then 
            ...
            if G.call('get_love',35) < 60 then 
                G.call("talk",'',35,'   我今天的功课还没有做完，以后有机会再说吧。师弟在少林修行只有一年的时间，光阴如箭，师弟要好好把握机会啊。',2,1) 
            else
                if G.call('get_love',35) < 75 then 
                    G.call("talk",'',35,'   师兄往日除了念佛诵经，其他一无所长，见识远不如师弟广博，因此实在不知从何聊起。',2,1) 
                else  
            ...
            G.call('all_over')   
        elseif int_选项 == 2 then 
            G.call("talk",'',35,'   师弟咱们点到为止，师兄的功夫可是稀松平常得很。',2,1) 
            G.call('all_over')
            ...
            if o_battle_结果  == 1 then
                ...
                G.call("talk",'',35,'   师弟的功夫果然大有长进，做师兄的真是自愧不如。',2,1) 
            else 
        ...
        elseif int_选项 == 3 then
            ...
            if G.call('get_point',11) < 3 then
                G.call("talk",'',35,'   原来师弟对佛法的了解还知之甚少，不过没关系，多去【藏经阁】向无名老僧请教，一定会使你受益匪浅。',2,1) 
                G.call('add_point',107,1)
            elseif G.call('get_point',11) < 5 then
                G.call("talk",'',35,'   原来师弟对佛法的了解还知之甚少，不过没关系，多去【藏经阁】向无名老僧请教，一定会使你受益匪浅。',2,1) 
                G.call('add_point',107,2)
            elseif G.call('get_point',11) < 7 then
                G.call("talk",'',35,'   师弟的在佛法修为上某处见解真是独到深刻，实在是领师兄拜服。',2,1) 
                G.call('add_point',107,math.random(2))
            elseif G.call('get_point',11) > 9 then
                G.call("talk",'',35,'   高明之至，师弟果然心有慧根，你的修为要比我精湛得多了，看来在这辈师兄弟当中，论佛学修为属你第一！',2,1) 
...
t['初入少林-缘根'] = function()
    ...
    local int_选项 = 0
	while int_选项 == 0 do
        int_选项 = G.call("menu",'',0,'',0,0,{"1,师兄这里有什么事情，师弟可以代劳的","2,想与师兄切磋一下武功","3,跟师兄打声招呼便回去了"},0) 
        if int_选项 == 1 then 
            G.call('all_over')  
            ...
            if G.call('get_love',206) > 80 and math.random(3) == 1 then 
                G.call("talk",'',206,'   这几日多亏了师弟帮忙，不然我这做师兄的也得挨那群老和尚训斥了，我这私藏了几颗灵丹妙药【玉灵散】和【六阳正气丸】，今天都给了你吧。',2,1) 
                ...
                G.call("talk",'',206,'   嗯，师弟辛苦了，早点回去休息吧 ',2,1) 
            end     
            G.call('all_over') 
            G.call('turn_map') 
        elseif int_选项 == 2 then
            ...
            if o_battle_结果  == 1 then
                ...
                G.call("talk",'',206,'   师弟果然天份奇高，想必是窥视到了我派的上乘武功，有机会还要向师弟多多请益！',2,1) 
            else 
                G.call('add_love',206,-5)
                G.call("talk",'',206,'   咱少林武功历来都是天下第一，你师兄我虽没进罗汉堂达摩院，但是平常趁他们练拳留心瞄上几眼就已受益匪浅，说来也有十几年的功力了，你这小师弟怎会是对手呢。',2,1) 
            end  
            G.call('all_over')
            G.call('turn_map') 
        elseif int_选项 == 3 then
            ...
            if o_battle_结果  == 1 then
                ...
                if o_battle_结果  == 1 then
                    ...
                    if o_battle_结果  == 1 or o_battle_结果  == 2 then
                        G.call("talk",'',35,'   哈哈,闯过【木人巷】的师弟果然是今非昔比！咱们就此别过，后会有期...',2,1) 
                    end  
                    G.call('初入少林-选择') 
                    G.call('all_over') 
                elseif o_battle_结果 == 2 then
