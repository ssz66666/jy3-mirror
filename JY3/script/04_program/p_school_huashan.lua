...
t['初入华山'] = function()
    ...
    G.call("talk",'',191,'   冲儿放肆！'..G.QueryName(0x10030001)[tostring(2)]..'刚入华山你就要教他不学好吗？你身为华山大弟子不以身作则，还要影响[师弟]们，你这成什么体统！',2,1) 
    G.call("talk",'',2,'   是，师父，弟子知错了。',2,1)
    G.call("talk",'',24,'   好啦好啦，大师兄你啊每次认错都认得诚恳，忘起教训来却比谁都快。嘻嘻，我又添个小[师弟]啦，爹爹这次下山回来一下给我带来两个师弟，我这个师姐可够威风啦。',2,1)
    G.call("talk",'',31,'   '..G.QueryName(0x10030001)[tostring(1)]..'[师弟]你好，感谢你在我爹爹最后时刻替我送了他老人家一程，我林平之算是欠你这份人情，日后定当报答。',2,1)
...
t['初入华山-令狐冲1'] = function()
	...
	while int_选项 == 0 do
        ...
        elseif int_选项 == 2 then
            G.call("talk",'',2,'   正是闲的无聊，[师弟]快来陪我活动活动筋骨。',2,1) 
            G.call('all_over')
            ...
            if o_battle_结果  == 1 then
                G.call('add_love',2,2)
                G.call("talk",'',2,'   [师弟]照这个进度练下去，我这个大师兄可就抵挡不住了。咱华山的小[师弟]一个比一个有出息，大师兄我也觉得脸上有光彩。',2,1) 
            else 
                G.call('add_love',2,-2)
                G.call("talk",'',2,'   [师弟]闪躲跳跃身形滞涩，出招发力既飘且虚，想必是本派的轻功和内功还没有练到家的缘故，所以有此一败！',2,1) 
            end  
            G.call('all_over')
            G.call('add_time',4)
            G.call('turn_map')  
...
t['初入华山-岳灵姗'] = function() 
	...
	while int_选项 == 0 do
        ...
        elseif int_选项 == 3 then 
            if math.random(3) == 1 then 
                G.call("talk",'',24,'   凭你也配来数落我？就连我爹妈也不肯说重我半句，你算什么啊，刚入门的小[师弟]就目无尊长吗！',2,1)
                G.call('add_love',24,-5) 
            else
                G.call("talk",'',24,'   好吧好吧，[师弟]是我错了，我以前太任性了，全失了做师姐的身份。以后我一定好好待你，师姐之前若有什么对不住还请[师弟]多多包涵',2,1) 
                G.call('add_love',24,5)
            end 
            G.call('all_over')
        elseif int_选项 == 4 then 
...
t['初入华山-林平之'] = function() 
    if G.call('get_story',10) == 0 then 
        ...
        G.call("talk",'',31,'   '..G.QueryName(0x10030001)[tostring(1)]..'[师弟]找我有什么事情吗？',2,1)
        ...
        G.call("talk",'',31,'   我只知道福州向阳巷却有一处我林家老宅，只是那里有什么物事我却不知道了，等有机会我去看个明白。多谢[师弟]了。',2,1)
        ...
        while int_选项 == 0 do
            ...
            if int_选项 == 1 then 
                G.call("talk",'',31,'   平之资质甚浅，请[师弟]多多指教。',2,1) 
                G.call('all_over')
                ...
                if o_battle_结果  == 1 then
                    ...
                    G.call("talk",'',31,'   [师弟]好本事啊，你练功夫的进展可比我快多了。像我这么笨的，不知道什么时候才能练好武功报灭门之仇...',2,1) 
                else 
                    G.call('add_love',31,-3)
                    G.call("talk",'',31,'   是[师弟]手下留情还是我真的有所成就了？这样看来，我手刃仇人也不是没有希望了。',2,1) 
                end  
                G.call('all_over')
                G.call('add_time',4)
                G.call('turn_map') 
    ...
    if G.call('count_day') ~= G.QueryName(0x10030001)[tostring(142)] then 
        ...
        elseif G.call('get_story',12) == 1 then 
            ...
            elseif G.call('get_magicexp',178) == 0  then 
                ...
                G.call("talk",'',75,'   短短时间你的所学已经远远超过了我的指望。岳不群那[小子]，教徒弟的本事不行，选徒弟的能耐倒是不错。以后有机会你见到我风师兄，再跟他学功夫吧。我在这山上也呆得够久了，该去云游四方了。',2,1)
            ...
            if G.call('get_item',95) == 0 and  G.call('get_magic',149) == 0 then 
                ...
                while int_选项 == 0 do
                    ...
                    if int_选项 == 1 then 
                        ...
                        G.call('set_role',2,15,G.call('get_role',2,1))
        ...
        while int_选项 == 0 do
            ...
            elseif int_选项 == 2 then 
                if G.call('get_item',194) > 0 then
                    ...
                    G.call("talk",'',2,'   哈哈，好[小子]！到底是你最对我的胃口啦。真是稀奇啊，华山门规这么严，你竟然能搞来几瓶酒，本事和胆子都挺大嘛。不错，我喜欢。',2,1)
                    G.call('add_love',2,5)
                else
                    ...
                    if G.call('get_story',11) == 0 then 
                        ...
                        while int_选项 == 0 do
                            ...
                            if int_选项 == 1 then 
                                ...
                                G.call('set_role',27,15,G.call('get_role',2,1))
                            ...
                            if G.call('get_story',11) == 1 then 
                                G.call("talk",'',2,'   [师弟]真行啊，你到学会顺手牵羊反手套猪的本事啦！你得到的这些兵器其实是当年攻打我华山的魔教长老的，他们后来被五岳派各位前辈用计引到思过崖上想聚而歼之，',2,1)
...
t['初入华山-出师'] = function() 
    ...
    if G.call('get_love',24) >= 80 then 
        G.call("talk",'',24,'   爹啊，您常说读万卷书走万里路，我想借这个机会跟[师弟]一起下山到江湖上浏览一番，[师弟]什么都不懂，没我这个师姐带路怕是要碰钉子呢',2,1)
        ...
        if G.call('get_love',31) >= 80 then 
            G.call("talk",'',31,'   启禀师父，徒儿也想借这个机会跟[师弟]一起下山，回福建探访我林家的老宅，以完成父亲的遗命。',2,1)    
            if G.call('team_full') == false then 
                G.call('join',31)
