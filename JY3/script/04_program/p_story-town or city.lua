...
t['城镇-京城']=function()
    if G.call('get_story',45) == 0 then 
        ...
        G.call("talk",'',220,'   滚开臭[小子]，不要挡我【段天德】大老爷的官路！',1,1) 
        ...
        if o_battle_结果  == 1 then
            G.call("talk",'',220,'   好[小子]，你给我等着，我去找人来教训你！',1,1) 
            G.call('all_over')
            G.call('dark')
        ...
        while int_选项 == 0 do
            ...
            if int_选项 == 1 then
                ...
                if G.QueryName(0x10030001).性别 == 1 then 
                    ...
                    G.call("talk",'太  监',226,'   等你下辈子再来吧！',1,1) 
                end
            elseif   int_选项 == 2 then 
                G.call('all_over') 
            end
...
t['城镇-梅庄']=function()
   if G.call('get_story',18) == 1 then 
        ...
        G.call("talk",'',17,'   小[兄弟]，咱们又见面了！',1,1) 
        G.call("talk",'',0,'   向大哥邀我前来，可有什么事情叫[小弟]帮忙？',0,0) 
        G.call("talk",'',17,'   不可多问。你只须听我的吩咐，照我的安排去做就好了。',1,1) 
        G.call('all_over') 
        G.call('dark')
        G.call("talk",'',17,'   【嵩山派】童化金拜见梅庄几位前辈！',1,1) 
        G.call("talk",'',188,'   童化金？没听说过。你到梅庄有何贵干？',1,1) 
        G.call("talk",'',17,'   我们此番前来，只为和各位前辈打一个赌，我赌你们这梅庄内无人能胜得过我这位[兄弟]！倘若我们输了，我这《溪山行旅图》《广陵散》等诸宝物，就献于梅庄各位。',1,1) 
...
t['城镇-密林']=function()
    if G.call('get_story',39) > 0 or G.call('get_item',261) == 0 or  G.call('get_story',40) == 1 then 
        ...
        G.call("talk",'女  童',115,'   傻[小子]，快背我跑！跑的越远越好！',1,1)
        ...
        if G.call('get_point',18) >= 80 then--判断悟性高于80触发梦姑剧情
            ...
            G.call("talk",'',115,'   傻[小子]，好好享受吧！',1,1) 
...
t['城镇-无量山洞']=function()
    if G.call('get_story',44) == 0  and  G.call('in_team',9) == false then
        ...
        G.call("talk",'',9,'   原来[公子]是跟我刚入洞时一样的困惑。不过在我后来得知这洞里原住一对神仙眷侣，后来男的照那女的模样刻了这玉像...',1,1) 
        if G.call('team_full') == false then 
            G.call('join',9)
            G.call("talk",'',9,'   其实我是被人到处追杀，走投无路才躲到这里的，也算是和这【神仙姐姐】般的玉像一种缘分吧。我看[公子]看似会武功的样子，请带我离开这吧，我可真怕那些恶人什么时候会找了进来。',1,1) 
        else
            G.call("talk",'',9,'   其实我是被人到处追杀，走投无路才躲到这里的，也算是和这【神仙姐姐】般的玉像一种缘分吧。',1,1)  
        end
       
        if G.call('get_point',15) > 29 then
            G.call("talk",'',9,'   我从【神仙姐姐】姐姐那里得到两本秘籍，《凌波微步》和《北冥神功》。我生性不喜习武，我看[公子]是侠义之人，武功越强，则所做善事越大。所以我决定把这两本秘籍赠送给你。',1,1) 
...
t['城镇-树林']=function()
    if G.call('get_magicexp',94) == 0   and  G.call('team_full') == false and  G.call('in_team',29) == false  then 
        G.call('dark')
        G.call("talk",'',29,'   [公子]快救救我！有人要追杀我！',1,1)  
        G.call('all_over') 
        G.call('add_time',2)
        G.call('call_battle',1,20,3,50,(230-224)*6+265,229,230,0,0,0)
        local o_battle_结果 = G.call('get_battle') 
        if o_battle_结果  == 1 then
            G.call("talk",'',29,'   多谢[公子]救我性命。我叫游坦之，今后就是[公子]的仆从了，现在世道人心险恶，要是没人罩着，活得还真不会长久。我有一套祖传的【南山掌法】传授给你吧。',1,1) 
            G.call('join',29)
            G.call('learnmagic',94)
        else
            G.call("talk",'',29,'   烦请[公子]快去找救兵，务必要救我一救啊！',1,1) 
        end 
    else
    ...
    if G.call('in_team',36) == false  then 
        ...
        if o_battle_结果  == 1 then
            if  G.call('get_point',15) > 59 and  G.call('get_magicexp',217) == 0 and G.call('get_story',57) ~= 1 then 
                G.call("talk",'',36,'   小[兄弟]，你武功既高，人品又不错，我传你一套【降龙十八掌】，望你能更好的造福于武林。',1,1)
...
t['城镇-苗人凤居']=function()
    ...
    if G.call('get_story',41) == 0  then
        G.call("talk",'',102,'   【苗人凤】你已中了【天下第一奇毒】！现在你双目失明，几天后就会全身溃烂而死，就算你有通天彻地的武功，也是难逃一死！看热闹的小[兄弟]，咱们一起联手除掉这个瞎子吧。',1,1) 
        local int_选项 = 0 
        while int_选项 == 0 do
            ...
            if int_选项 == 1 then
                ...
                local o_battle_结果 = G.call('get_battle') 
                if o_battle_结果  == 1 then
                    G.call("talk",'',102,'   好[小子]，原来你也觊觎那份藏宝图。既然你武功高强，我们是没话说了，不过这苗人凤也快死了，他肯不肯拿给你就看你的造化了，哈哈哈...',1,1)  
                    G.call('add_point',15,5)
                    G.call("talk",'',156,'   小[兄弟]，我中的毒，只有【药王庄】的传人才能医治的了，以苗某的功力暂时还挺得住，烦请小[兄弟]去请药王庄的传人来医治我的眼睛，苗某感激不尽。',1,1) 
            ...
            elseif   int_选项 == 2 then 
                ...
                if o_battle_结果  == 1 then
                    ...
                    G.call('add_point',14,50)
                    G.call('set_story',41,3)
                    G.call('set_note','杀死号称【打遍天下无敌手】之苗人凤！')
                    G.call('all_over') 
                    G.call('add_time',2)
    ...
    elseif G.call('get_story',41) == 1  then
        if G.call('in_team',40) == true  then 
            ...
            G.call('dark')
            G.call("talk",'',156,'   【药王庄】的弟子果然神乎其技！小[兄弟]，你救了苗某的性命，苗某没什么可报答的。不如指点你一招武功如何，这便是我的【苗家剑法】！',1,1) 
            if G.call('get_magicexp',47) == 0 then 
                G.call('learnmagic',47)
            end
            if G.call('get_point',15) > 49  then 
                G.call("talk",'',156,'   小[兄弟]，你身怀侠义心肠，武功又高，苗某的这份【闯王宝藏图】交给你保管是最适合不过了！他日你开启宝藏，获取里面财物，一定要用于正道，普度天下众生，否则便是辜负了苗某对你的一番信任！',1,1) 
                if G.call('get_item',264) == 0  then 
                    G.call('add_item',264,1)
                end 
            else
                G.call("talk",'',156,'   小[兄弟]，你今日虽然救我，但是我看出你眼中深藏戾气，希望你日后能走正路，否则祸到临头，你将悔之晚矣！',1,1)
            end    
        else
...
t['城镇-绿柳山庄']=function()
    if G.call('get_story',51) == 0 then 
        ...
        while int_选项 == 0  do
            ...
            if int_选项 == 1 then
                G.call("talk",'',44,'   阿二先来领教[公子]的拳法',1,1)
                G.call('all_over') 
                G.call('add_time',2)
                G.call('call_battle',1,10,1,100,44,0,0,0,0,0)
                local o_battle_结果 = G.call('get_battle') 
                if o_battle_结果  == 1 then
                    G.call("talk",'',43,'   阿大来领教[公子]的剑法',1,1)
                    G.call('all_over') 
                    G.call('add_time',2)
                    G.call('call_battle',1,10,1,100,43,0,0,0,0,0)
                    local o_battle_结果 = G.call('get_battle') 
                    if o_battle_结果  == 1 then
                        G.call("talk",'',15,'   [公子]武功高强，赵敏今日得见，请到这边先用些酒饭。',1,1)
                        ...
                        G.call("talk",'',15,'   这个稍后再说，小妹不胜酒力，再饮恐有失仪，现下说话已不知轻重了。我进去换一件衣服，片刻即回，请[公子]自便，不必客气。',1,1)
                        G.call('all_over') 
                        local int_选项 = 0 
                        while int_选项 == 0  do
                            ...
                            if int_选项 == 1 then
                                ...
                                G.call("talk",'',15,'   [公子]果然好见识，也是好本事竟然没有中毒。',1,1)
                                G.call("talk",'',0,'   赵姑娘好算计，我与姑娘并无冤仇何苦如此？',0,0)
                                G.call("talk",'',15,'   这个就要问[公子]自己了，本姑娘今天还有要事，恕不奉陪。。。',1,1)
                                ...
                                G.call("talk",'',15,'   多承[公子]夸赞，小女子愧不敢当。',1,1)
                                ...
                                G.call("talk",'',15,'   臭[小子]……贼……[小子]，总有一天，我……我将你千刀……千刀万剐……好啦，好啦，饶……饶了我罢……[公子]……呜呜……呜呜……',1,1)
                                G.call("talk",'',0,'   你放不放我？',0,0)
                                G.call("talk",'',15,'   我……放……快……停手……',1,1)
                                G.call("talk",'',0,'   得罪了！',0,0)
                                G.call("talk",'',15,'   贼[小子]，给我着好鞋袜！',1,1)
                                G.call("talk",'',0,'   赵姑娘，适才在下实是迫于无奈，这里跟你谢罪了。',0,0)
                                G.call('dark')
                                G.Play(0x49010039, 1,true,1) 
                                if G.call('team_full') == false then 
                                    G.call("talk",'',15,'   不过以后我要跟[公子]一起，不知[公子]可否应了',1,1)
                                    G.call("talk",'',0,'   好吧，我们先离开这里',0,0)
                                    G.call('join',15)
                                else
                                    G.call("talk",'',15,'   [公子]以后有需要不妨再来绿柳山庄！',1,1)
                                end
                            elseif int_选项 == 2 then   
                                ...
                                if o_battle_结果  == 1 then
                                    ...
                                    G.call("talk",'',252,'   [公子]见笑!',1,1)
                                    G.call("talk",'',0,'   我们看看怎么离开这里',0,0)
                                    G.call("talk",'',252,'   [公子]看这里。。。',1,1)
                                    G.call("talk",'',0,'   竟然开关在这里，我们先离开。',0,0)
                                    G.call('dark')
                                    G.Play(0x49010039, 1,true,1) 
                                    G.call("talk",'',0,'   小昭你刚刚唱的是什么？',0,0)
                                    if G.call('team_full') == false then 
                                        G.call("talk",'',252,'   只要[公子]喜欢听，以后小昭都会唱给[公子]听。',1,1)
                                        G.call('join',252)
                                    else
                                        G.call("talk",'',252,'   多谢[公子]今日相助，我还要去找我们教主，就此别过！',1,1)
                                    end 
                                else
                        ...
                        G.call("talk",'',15,'   [公子]看来是看不成倚天剑了，就请速速离开！',1,1)
    ...
    elseif G.call('get_story',51) == 2 then 
        if G.call('in_team',15) == true then 
            ...
            if G.call('team_full') == false then 
                G.call("talk",'',15,'   只要[公子]需要，赵敏愿意一直陪伴[公子]！',1,1)
                G.call('join',15) 
            else
                G.call("talk",'',15,'   [公子]好像现在并不需要赵敏追随！',1,1)
            end
        end
    elseif G.call('get_story',51) == 3 then 
        if G.call('in_team',252) == true then 
            ...
            G.Play(0x49010038, 1,true,1) 
            G.call("talk",'',252,'   只要[公子]想听小昭现在就唱给[公子]听。',1,1)
        else
