        ...
        if int_选项 == 1 then 
            if #brother == 0 then 
                ...
                elseif G.misc().brother == 999 then 
                    ...
                    local int_选项 = 0
                    while int_选项 == 0 do
                        G.call("talk",'',38,'   与【'..G.QueryName(0x10040000+G.misc().brother).姓名..'】结义吗？你可要认真考虑，机会只有一次，无论错过还是草率结义都是来不及后悔的。成为你结义'..str_字符串..'，各项属性均会大涨，成为你打架斗殴时的好帮手，可要慎重考虑！',1,1) 
...
t['聚贤庄任务_擂鼓山棋局']=function()
    ...
    if G.call('in_team',35) == false  and  (G.call('get_point',8) == 0 or G.call('get_point',8) == 2)  then 
        ...
        if o_battle_结果  == 1 then 
            ...
            G.call("talk",'',104,'   恭喜小[兄弟]当上掌门人。适才多谢你救我一命，我这【一阳指】就传授给你吧。',1,1) 
...
t['聚贤庄任务_天王老子傲四方']=function()
    ...
    while int_选项 == 0 do
        ...
        if int_选项 == 1 then
            ...
            o_battle_结果 = G.call('get_battle')
            if o_battle_结果  == 1 then 
                G.call("talk",'',17,'   好[兄弟]，今天咱们会斗天下英雄，当真是畅快之极。不过我还有要事在身，不能久留！[兄弟]若还想帮我，就到【梅庄】来找我。',1,1) 
                ...
                G.call("talk",'',17,'   小[兄弟]你快走吧，这个忙你是帮不上的！',1,1) 
            end
        elseif int_选项 == 2 then 
            G.call('all_over')
...
t['聚贤庄任务_群寇争镖银']=function()
    ...
    G.call("talk",'',157,'   这位过路的[兄弟]，帮田某把这小妞摆平，这镖车的银子咱们二一添作五如何？',1,1) 
    local int_选项 = 0
    while int_选项 == 0 do
        ...
        elseif int_选项 == 2 then 
            ...
            if o_battle_结果  == 1 then
                ...
                G.call("talk",'',157,'   [兄弟]好功夫，这些车银子都是[兄弟]的了。里面银子数目也未曾数过，这车体积最大，谅来装得的银子也该是最多的。',1,1) 
            else
                G.call("talk",'',0,'   大家都是出来混的，何必动刀动枪呢，伤了和气呢。如果一定要动的话，这里没我什么事了，等我走了你们再继续。',0,0) 
            end
        ...
        while int_选项 == 0 do
            ...
            elseif int_选项 == 3  then  
                ...
                if o_battle_结果  == 1 then
                    ...
                    if G.call('team_full') == false then 
                        G.call("talk",'',34,'   咱们既是同道中人，又何分彼此呢。以后[兄弟]的女人就是小王的女人，咱们有福同享！',1,1) 
                        G.call('join',34)
                    else
                    end     
                    G.call("talk",'',158,'   你们不要高兴的太早了，这个女娃终究还是我的囊中之物！',1,1) 
...
t['聚贤庄任务_死守襄阳城']=function()
    ...
    if o_battle_结果  == 1 then
        ...
        G.call("talk",'',37,'   今日多亏了'..G.QueryName(0x10030001)[tostring(1)]..'[兄弟]挺身而出！[兄弟]把满清的金轮国师打得重伤呕血，想必这数年间不会再兴兵南下了，襄阳百姓终可过上几天舒服日子，这全有赖于你呀！',1,1)
        if G.call('team_full') == false then 
...
t['聚贤庄任务_庄家的复仇']=function()
    ...
    if G.call('get_story',48) == 1 then 
        ...
        G.call('talk0','庄三少奶奶','   小[兄弟]...你是我们庄家的大恩人！我得想想怎么感谢你才好。',126,143)
        if G.call('team_full') == false then 
...
t['聚贤庄任务_昔日皇妃']=function()
    ...
    G.call("talk",'',86,'   我这百花谷好久没人来过了，我都快无聊死了。小[兄弟]，来来来，快陪我好好玩玩。',1,1) 
    ...
    if o_battle_结果  == 1 then
        ...
        G.call("talk",'',86,'   不要再说了。今日倘若不去见她，以后的日子别想再睡得着。小[兄弟]我跟你去见她...',1,1) 
        G.call('地图_进入地图','黑龙潭',50,33)
        G.call('add_point',15,15)
        G.call("talk",'',88,'   多谢小[兄弟]了了我多年的心愿。',1,1)
...
t['聚贤庄任务_悲情杨过']=function()
    ...
    if G.call('get_item',284) > 0 then 
        ...
        if G.call('team_full') == false then 
            G.call("talk",'',0,'   杨兄现在伤痛已好，可有什么未了的心愿，咱们可结伴同行，[兄弟]帮你来完成。',0,0) 
            G.call("talk",'',7,'   如果没有[兄弟]你，我现在已经是个死人了，还谈什么心愿不心愿的。总之[兄弟]一句话：刀里刀里去，火里火里去！',1,1) 
            G.call('join',7)
        else
            ...
            if o_battle_结果  == 1 then
                ...
                if o_battle_结果  == 1 then
                    ...
                    if o_battle_结果  == 1 then
                        ...
                        G.call("talk",'',122,'   小[兄弟]果然是少年英雄啊，今天一战必定扬名四海。我们就尊你是【武林盟主】了！',1,1) 
                        G.call('add_point',14,150)
                        G.call('set_CH','武林盟主')
            ...
            if G.call('get_point',8) == 7 then 
                ...
                if o_battle_结果  == 1 then
                    ...
                    while int_选项 == 0 do
                        ...
                        elseif int_选项 == 2 then
                            G.call('all_over')  
                            G.call("talk",'',140,'   小[兄弟]怎么不杀那个小的？',1,1) 
                            G.call("talk",'',0,'   这个小[兄弟]看着还不错，干嘛要杀掉？',0,0) 
                            G.call('add_point',14,75)
...
t['聚贤庄任务_镖车被劫']=function()
	...
	while int_选项 == 0 do
        ...
        elseif int_选项 == 2 then 
            ...
            o_battle_结果 = G.call('get_battle')
            if o_battle_结果  == 1 then 
                G.call("talk",'强  盗',228,'   多谢[兄弟]出手相助，这点谢礼不成敬意，务请收下。',1,1) 
